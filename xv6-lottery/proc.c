...
// lottery code
  ...
  for(;;){
	...
	// loop over process table looking for process to run
        acquire(&ptable.lock);
	//nice_total = get_nice_sum(); // or just run the code to get the nice sum here
	
	// this is running the code to get the nice sum instead of sending to a different function
	nice_total = 0;
	for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
		if(p->state == RUNNABLE){
			nice_total += p->nice_value;
		}
	}
		
	if(nice_total > 0){ 
		rand_num = rand(); // the rand function we made
		rand_winner = rand_num % nice_total + 1; // to keep the rand_num in the size
		nice_total = 0;
		...
		for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
			...
			nice_total += p->nice_value; // nice_value will be something we add to our struct

			if(nice_total < rand_winner){
				continue;
			}
...
renice(int pid, int nice){
	...
	for(i = 0; i < NPROC; i++){
		// if it exists, change TorF to true and break out of loop
...
//# error this is an excellent place to add some new data to the o/p of cps
	#ifdef LOTTERY
    	    cprintf("\t%u-%s%u-%s%u %s%u:%s%u:%s%u \t%d\t%d\t%d"
	...
	#else // LOTTERY
    	    cprintf("\t%u-%s%u-%s%u %s%u:%s%u:%s%u \t%d\t%d"
		, ptable.proc[i].begin_date.year
		, ptable.proc[i].begin_date.month < 10 ? "0" : "" 
		, ptable.proc[i].begin_date.month 
		, ptable.proc[i].begin_date.day < 10 ? "0" : ""
